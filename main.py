...
fmaGDF = gpd.GeoDataFrame(geometry = fma).set_crs(Visum.Net.AttValue('ProjectionDefinition'), allow_override = True).to_crs('WGS84')
fmaHex = fmaGDF.h3.polyfill_resample(9).to_crs(Visum.Net.AttValue('ProjectionDefinition'))
fmaHex['centroid'] = fmaHex['geometry'].centroid
fmaHexWKT = fmaHex.to_wkt()
...
MinIsocTimeAtStopAtt = 'Min:StopAreas\\Node\\IsocTimePrT'
...
mapMatcher = Visum.Net.CreateMapMatcher()
